...
import { Popover, PopoverButton, PopoverPanel, Dialog, Disclosure, Menu, MenuButton, MenuItem, MenuItems, Transition, PopoverBackdrop } from '@headlessui/react'
import {
  Bars3Icon,
  CursorArrowRaysIcon,
  FingerPrintIcon,
...
const Header: React.FC<HeaderProps> = ({ }) => {
  ...
  return (
    <>
      <nav className="bg-white fixed w-full p-6 shadow-sm lg:px-8" aria-label="Global">
        <div className=' relative isolate flex gap-5 items-center justify-between'>
          ...
          <Popover className="relative hidden lg:flex" >
            {({ open, close }) => (
              <>
                <PopoverButton className="bg-gray-50 px-4 py-2 rounded-xl hover:bg-gray-100 outline-none" onClick={({ target }) => !open ? "" : (target as HTMLElement).click()} onMouseEnter={({ target }) => open ? "" : (target as HTMLElement).click()}>Solutions</PopoverButton>
                ...
                <PopoverPanel
                  ...
                  <div className='grid grid-cols-12 gap-5 container'>
                    ...
                    <div className='col-span-3'>
                      <ul className='space-y-3'>
                        {selectedItem && childItems[selectedItem].map((child, index) => (
                          <li
                            key={index}
