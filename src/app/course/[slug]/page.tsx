    ...
    return {
      ...
      abstract:
        course.mini_description !== null
          ...
          : "Discover personalized online tutoring courses tailored to your learning needs. Our expert tutors provide one-on-one instruction in all subjects and grades, helping students achieve academic success from the comfort of their homes. Enroll now to boost your grades and confidence!",
      alternates: {
        canonical: "https://onlinetutoringcourses.com/course/" + course.slug,
      },
    };
  }
}
...
export default async function Page({ params, searchParams }: pageProps) {
  ...
  const pathname =
    process.env.NEXT_PUBLIC_WEBSITE_URL + "course/" + course.slug;
  return (
            ...
            <div className="mt-5 space-y-8">
              <div className="space-y-3">
                <h1 className="font-bold">
                  {course.name.length < 20
                    ? `${course.name} - Online Tutoring Course`
                    : course.name}
                </h1>
                ...
                <div className="space-y-2">
                  ...
                  <div className="gap-3 flex items-center flex-wrap">
                    {course.available_languages.map((lang) => (
                      <div
                        key={lang.languages_id.id}
                        className="flex items-center gap-2 bg-background rounded-xl px-6 py-3"
                      >
                        ...
                        <h3 className="text-gray-700 font-[500]">
                          {lang.languages_id.name}
                        </h3>
                      </div>
                    ))}
                  </div>
              ...
              {course.what_learn && (
                <div>
                  ...
                  <div
                    className="pl-5 custom-ul-li"
                    dangerouslySetInnerHTML={{ __html: course.what_learn }}
                  ></div>
                </div>
              )}
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
                    {course.related_courses.map((related) => (
                      <Card
                        key={related.related_courses_id.id}
                        course={related.related_courses_id}
                      />
                    ))}
                  </div>
                </div>
